...
import { X, Share2, Download, Play, Pause, Link } from 'lucide-react-native';
...
export function MediaViewerModal({
  ...
  const handleShare = async () => {
    ...
    const mimeType = isVideo
      ? `video/${fileExt === 'mkv' ? 'x-matroska' : fileExt}`
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      if (Platform.OS === 'ios') {
        Alert.alert(
          'Download Success',
          `Saved to Documents as ${sanitizedName}`,
        );
      } else {
      ...
      <View style={styles.container}>
        ...
        {headerVisible && (
          <View style={styles.footer}>
            <Pressable onPress={handleCopyLink} style={styles.footerButton}>
              <LucideIcon icon={Link} color="#fff" size={20} />
...
const styles = StyleSheet.create({
  ...
  snackbar: {
    backgroundColor: '#333',
    marginBottom: 110, // Show above the footer
  },
  loader: {
    position: 'absolute',
  },
